...
class ElectraForSequenceClassification(ElectraPreTrainedModel):
    def __init__(self, config):
        ...
        self.dropouts = nn.ModuleList([nn.Dropout(config.hidden_dropout_prob) for _ in range(self.dropout_size)])
    ...
    def forward(
        ...
        for i in range(self.dropout_size) :
            if i == 0 :
                logits = self.classifier(self.dropouts[i](sequence_output))
            else :
                logits += self.classifier(self.dropouts[i](sequence_output))
